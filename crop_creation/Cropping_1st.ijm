...
function processFile(input, output, file) {
	...
	run("Bio-Formats Importer", "open=" + inputFolder + File.separator + file + " autoscale color_mode=Default rois_import=[ROI manager] view=Hyperstack stack_order=XYCZT");
	
	imageName = getTitle();
	
	// get set number
	dirString = File.getParent(inputFolder + File.separator + file);
	dirArray = split(dirString, File.separator);
	dirArrayLength = dirArray.length - 1;
	baseDir = dirArray[dirArrayLength];
	
	setArray = split(baseDir, File.separator);
	setString = setArray[0];
	setStringArray = split(setString, "st");
	setNumber = setStringArray[0];
	
	// get name number
	fileString = File.getNameWithoutExtension(inputFolder + File.separator + file);
